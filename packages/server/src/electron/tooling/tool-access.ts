/**
 * Tool Accesses - Resource tracking and conflict detection
 *
 * Based on kimi-code's tool access system for:
 * - File operation tracking (read/write/search)
...
export const ToolAccesses = {
  ...
  browser(): ToolAccesses {
    return [{ kind: 'all' }]; // Browser operations access everything
  },
};
...
function resourceAccessesConflict(
  ...
  return fileAccessesOverlap(left as ToolFileAccess, right as ToolFileAccess);
}
