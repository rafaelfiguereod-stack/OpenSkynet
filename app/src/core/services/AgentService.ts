  ...
  async stream(
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        // Create wrapper for onProgress that validates data
        const safeOnProgress = (data: unknown) => {
          try {
            const validated = StreamProgressSchema.safeParse(data);
            if (validated.success) {
              callbacks.onProgress?.(validated.data);
            }
          } catch (error) {
            console.warn('Error in onProgress:', error);
          }
        };
