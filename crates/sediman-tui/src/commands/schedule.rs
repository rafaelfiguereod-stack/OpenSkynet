...
pub async fn handle_schedule(app: &mut App, _args: &str) {
    match app.connection.bridge.list_schedules().await {
        Ok(jobs) => {
            app.modals.schedule_jobs = jobs;
            app.modals.schedule_selected = 0;
            app.modals.schedule_scroll = 0;
            app.modals.schedule_input.clear();
            app.modals.active = Some(AppModal::ScheduleBrowser);
        }
        Err(e) => {
...
pub async fn handle_schedule_remove(app: &mut App, args: &str) {
    ...
    match app.connection.bridge.remove_schedule(args).await {
