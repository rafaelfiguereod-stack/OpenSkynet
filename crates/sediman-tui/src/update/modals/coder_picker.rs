...
pub async fn handle_coder_picker(app: &mut App, key: crossterm::event::KeyEvent) -> bool {
    match key.code {
        ...
        KeyCode::Up | KeyCode::Char('k') => {
            if app.modals.coder_picker_selected > 0 {
                app.modals.coder_picker_selected -= 1;
            }
            true
        }
        KeyCode::Down | KeyCode::Char('j') => {
            if app.modals.coder_picker_selected < CODER_BACKENDS.len() - 1 {
                app.modals.coder_picker_selected += 1;
            }
            true
        }
        KeyCode::Enter => {
            if let Some(backend) = CODER_BACKENDS.get(app.modals.coder_picker_selected) {
                let old = app.agent.coder_backend.clone();
                app.agent.coder_backend = backend.to_string();
                if old != app.agent.coder_backend {
                    // Persist
